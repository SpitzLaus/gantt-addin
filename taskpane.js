    ...
    await PowerPoint.run(async (context) => {
      ...
      model.forEach((row, i) => {
        ...
        if (row.kind === "group") {
          ...
          g.name = "GanttGroup";
          created.push(g);
          created.push(addText(shapes, row.name, MARGIN.left, rowTop, LABEL_WIDTH + 200, L.rowH,
            { size: 11, bold: true, color: "#1b1b1f", valign: "middle" }));
          return;
        }

        // Task label (indented to the right, below the group heading)
        created.push(addText(shapes, row.name, MARGIN.left + 18, rowTop, LABEL_WIDTH - 26, L.rowH,
      ...
      if (showToday && today >= start && today <= end) {
        ...
        created.push(addText(shapes, "Heute", tx + 2, plotBottom + 4, 40, 12, { size: 8, color: "#C00000" }));
      }
...
// Requirement 1+2: original routed dependency arrow. The vertical channel sits
// in the white space just right of the source; the line body is drawn as thin
// axis-aligned rectangles and the head is a rotated triangle at the target.
function drawDependency(shapes, created, a, b, style, color, rowH) {
  ...
  // links, sodass der Pfeil nach rechts in das Ziel zeigt.
  if (x2 < x1) {
    const d = x1 - x2;
    const approachX = x2 - 2 * d;
    const midY = (y1 + y2) / 2;
...
// Triangular arrowhead (rotated to face travel direction).
