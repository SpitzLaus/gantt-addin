  ...
  try {
    ...
    await PowerPoint.run(async (context) => {
      ...
      buildPeriods(start, end, finest).forEach((p) => {
        ...
        addVLine(shapes, created, x, L.plotTop, plotBottom, "#E3E3E6", { weight: 0.5 });
      });
      ...
      if (showToday && today >= start && today <= end) {
        const tx = xFor(today);
        addVLine(shapes, created, tx, L.plotTop - 4, plotBottom + 4, "#C00000", { weight: 1.25, dash: true });
...
// Requirement 1+2: dependency drawn as ONE genuine native PowerPoint arrow
// auto-shape with a real, built-in arrowhead — never assembled from separate
// rectangles/triangles. Because PowerPoint's connector/LineFormat API exposes
// no arrowhead properties, a real arrowhead can only come from a native arrow
// shape. For an elbow (right-angle) route we use GeometricShapeType.bentArrow,
// a single L-shaped arrow object; for same-row dependencies (no vertical
// offset) an elbow is meaningless, so we fall back to a straight rightArrow.
function drawDependency(shapes, created, a, b, style, color, rowH) {
  ...
  const thick = 6;                       // arrow shaft thickness in points

  // Same-row (or almost): a plain straight horizontal arrow with a real head.
  if (Math.abs(dy) < thick) {
    const w = Math.max(Math.abs(dx), 10);
    const arrow = shapes.addGeometricShape(PowerPoint.GeometricShapeType.rightArrow, {
      left: Math.min(sx, tx), top: sy - thick / 2, width: w, height: thick,
    });
    arrow.fill.setSolidColor(color);
    arrow.lineFormat.visible = false;
    arrow.name = "GanttLink";
    try { arrow.flipHorizontal = tx < sx; } catch (e) { /* flip unsupported */ }
    created.push(arrow);
    return;
  }

  // Elbow route: single native bent arrow spanning the S→T bounding box.
  // The default bentArrow travels right, then bends and points UP with its
  // head at the top. We flip it so the shaft leaves the source and the head
  // enters the target, whatever their relative positions.
  const left = Math.min(sx, tx);
  const top = Math.min(sy, ty);
  const w = Math.max(Math.abs(dx), 12);
  const h = Math.max(Math.abs(dy), 12);
  const arrow = shapes.addGeometricShape(PowerPoint.GeometricShapeType.bentArrow, {
  ...
  arrow.name = "GanttLink";
  try {
    // bentArrow points upward by default: head at top, tail entering from the
    // bottom-left. Flip vertically when the target is BELOW the source so the
    // head points down; flip horizontally when the target is to the LEFT.
    arrow.flipVertical = ty > sy;
    arrow.flipHorizontal = tx < sx;
  } catch (e) { /* flip unsupported on this host */ }
  created.push(arrow);
}
...
// Guaranteed-vertical line drawn as a thin rectangle (an addLine connector
// renders diagonally across its bounding box on some hosts, so we avoid it).
// For a dashed look (today marker) we stack short rectangles vertically.
// All created shapes are pushed into `created` so they get grouped/cleaned up.
function addVLine(shapes, created, x, y1, y2, color, opts = {}) {
  const t = opts.weight || 0.75;
  const top = Math.min(y1, y2);
  const total = Math.abs(y2 - y1);

  if (opts.dash) {
    const dash = 5, gap = 3;
    let yy = top;
    while (yy < top + total) {
      const h = Math.min(dash, top + total - yy);
      const r = shapes.addGeometricShape(PowerPoint.GeometricShapeType.rectangle,
        { left: x - t / 2, top: yy, width: t, height: h });
      r.fill.setSolidColor(color); r.lineFormat.visible = false; r.name = "GanttGrid";
      created.push(r);
      yy += dash + gap;
    }
    return;
  ...
  const rect = shapes.addGeometricShape(PowerPoint.GeometricShapeType.rectangle,
    { left: x - t / 2, top, width: t, height: total });
