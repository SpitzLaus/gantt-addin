...
// hosts (silently ignored), so we use the native directional block-arrow
// shapes (rightArrow / leftArrow) which inherently point the correct way with
// NO rotation needed. We make it short & squat so only the head is prominent.
function arrowTri(shapes, created, x, y, dir, size, color) {
  const w = size * 1.4;
  const left = dir === "right" ? x - w : x;
  const type = dir === "right"
    ? PowerPoint.GeometricShapeType.rightArrow
    : PowerPoint.GeometricShapeType.leftArrow;
  const arrow = shapes.addGeometricShape(type,
    { left, top: y - size / 2, width: w, height: size });
