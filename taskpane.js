...
// Requirement 1+2: dependency arrow drawn as ONE genuine native PowerPoint
// block-arrow auto-shape (GeometricShapeType.rightArrow). Because PowerPoint's
// connector/LineFormat API exposes no arrowhead properties, a real arrowhead
// can only be obtained from a native arrow shape. We take a single rightArrow,
// size its width to the source→target distance and rotate it to point exactly
// at the target, so the arrowhead (the shape's built-in point) lands on the
// successor. This is a real arrow with a real point — not assembled from
// separate rectangles and triangles.
function drawDependency(shapes, created, a, b, style, color, rowH) {
  ...
  // Place the arrow's (unrotated) bounding box centred on the S→T midpoint,
  // pointing right by default, then rotate around that centre to aim at T.
  ...
  try { arrow.rotation = Math.atan2(dy, dx) * 180 / Math.PI; } catch (e) { /* rotation unsupported */ }
  created.push(arrow);
}
