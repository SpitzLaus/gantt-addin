  ...
  try {
    ...
    await PowerPoint.run(async (context) => {
      ...
      buildPeriods(start, end, finest).forEach((p) => {
        ...
        created.push(addVRect(shapes, x, L.plotTop, plotBottom, "#E3E3E6", 0.75));
      });
      ...
      if (showToday && today >= start && today <= end) {
        const tx = xFor(today);
        addDashedVLine(shapes, created, tx, L.plotTop - 4, plotBottom + 4, "#C00000", 1.5);
        created.push(addText(shapes, "Heute", tx + 2, L.plotTop - 14, 40, 12, { size: 8, color: "#C00000" }));
      }

      // ---- Dependency arrows (routed through white space, one group each) ----
      ...
      links.forEach((link) => {
        ...
        if (!a || !b) return;
        const parts = [];
        drawDependency(shapes, parts, a, b, link.style, link.color);
        if (parts.length) connectorGroups.push(parts);
      });

      await context.sync();

      // Group each connector's segments + tip into a single arrow object.
      connectorGroups.forEach((parts) => {
        try {
          const g = shapes.addGroup(parts);
          g.name = "GanttLink";
          created.push(g);
        } catch (e) {
          // Grouping unsupported: keep the individual parts in the chart.
          parts.forEach((p) => created.push(p));
        }
      });
      if (connectorGroups.length) await context.sync();
...
// Requirement 1+2: an orthogonal arrow routed entirely through the white space
// (never diagonal, never cutting through a bar) with a real triangular arrow
// tip. All parts are pushed into `out`; the caller groups them into ONE object,
// so it behaves like a single arrow shape.
function drawDependency(shapes, out, a, b, style, color) {
  let x1, x2, enterFromLeft;
  switch (style) {
    case "start-start": x1 = a.left; x2 = b.left; enterFromLeft = true; break;
    case "finish-finish": x1 = a.right; x2 = b.right; enterFromLeft = false; break;
    default: x1 = a.right; x2 = b.left; enterFromLeft = true; break; // finish-start
  }
  const y1 = a.mid, y2 = b.mid;
  const pts = routePoints(x1, y1, x2, y2, enterFromLeft);

  // Draw each straight segment as a thin rectangle (guaranteed axis-aligned).
  for (let i = 0; i < pts.length - 1; i++) {
    const [ax, ay] = pts[i];
    const [bx, by] = pts[i + 1];
    if (Math.abs(ax - bx) < 0.5 && Math.abs(ay - by) < 0.5) continue;
    if (Math.abs(ay - by) < 0.5) hSeg(shapes, out, ax, bx, ay, color);
    else vSeg(shapes, out, ax, ay, by, color);
  }

  // Arrow tip at the final point, pointing along the last segment.
  const last = pts[pts.length - 1];
  const prev = pts[pts.length - 2];
  arrowHead(shapes, out, last[0], last[1], last[0] >= prev[0] ? "right" : "left", color);
}

// Orthogonal routing that leaves the source sideways, travels through the row
// gap (white space) and approaches the target from the chosen side.
function routePoints(x1, y1, x2, y2, enterFromLeft) {
  const stub = 8;
  const yGap = (y1 + y2) / 2;                 // lies on a row boundary = white
  const leaveX = enterFromLeft ? x1 + stub : x1 - stub;
  const approachX = enterFromLeft ? x2 - stub : x2 + stub;
  return [
    [x1, y1],
    [leaveX, y1],
    [leaveX, yGap],
...
function hSeg(shapes, out, xa, xb, y, color) {
  const t = 1.5;
...
function vSeg(shapes, out, x, ya, yb, color) {
  const t = 1.5;
...
// A proper triangular arrowhead pointing along the travel direction.
function arrowHead(shapes, out, x, y, dir, color) {
  const size = 8;
...
// Guaranteed-vertical line via a thin rectangle (addLine renders diagonally
// for zero-width bounding boxes, so we avoid it entirely here).
function addVRect(shapes, x, y1, y2, color, weight) {
  const t = weight || 1;
  const r = shapes.addGeometricShape(PowerPoint.GeometricShapeType.rectangle,
    { left: x - t / 2, top: Math.min(y1, y2), width: t, height: Math.abs(y2 - y1) });
...
// Dashed vertical line built from short vertical rectangles (kept vertical).
function addDashedVLine(shapes, created, x, y1, y2, color, weight) {
  const t = weight || 1.25;
  const dash = 5, gap = 3;
  let y = Math.min(y1, y2);
  const end = Math.max(y1, y2);
  while (y < end) {
    const h = Math.min(dash, end - y);
    const r = shapes.addGeometricShape(PowerPoint.GeometricShapeType.rectangle,
      { left: x - t / 2, top: y, width: t, height: h });
    r.fill.setSolidColor(color);
    r.lineFormat.visible = false;
    r.name = "GanttGrid";
    created.push(r);
...
function renderPreview() {
  ...
  readLinks().forEach((lk) => {
    ...
    if (!a || !b) return;
    let ax, bx, enterFromLeft;
    switch (lk.style) {
      case "start-start": ax = a.left; bx = b.left; enterFromLeft = true; break;
      case "finish-finish": ax = a.right; bx = b.right; enterFromLeft = false; break;
      default: ax = a.right; bx = b.left; enterFromLeft = true; break;
    }
    const ay = a.mid, by = b.mid;
    const pts = routePoints(ax, ay, bx, by, enterFromLeft);
